...
/// Get the platform-specific Claude Code config path
fn get_config_path() -> Result<PathBuf> {
    if cfg!(target_os = "windows") {
        ...
        // Unix (Linux/macOS): ~/.config/claude-code/config.json
...
/// Read existing config or create a new one with default structure
fn load_config(config_path: &Path) -> Result<Value> {
    if config_path.exists() {
        ...
        // Create new config with base structure
        Ok(json!({ "mcpServers": {} }))
    }
}
...
fn update_mcp_config(
    ...
    // Update or create demongrep entry
...
/// Write updated config back to file, creating directories if needed
...
///
/// # Arguments
/// * `global` - If true, use global config paths; if false, use current project
///
/// # Returns
...
pub fn run(global: bool) -> Result<()> {
    ...
    // Update with demongrep MCP server configuration
    ...
    println!("✅ Claude Code MCP integration installed successfully!");
    println!();
    ...
    println!();
    println!("📝 Configuration saved. Please restart Claude Code for changes to take effect.");
